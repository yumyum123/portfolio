$(document).ready(function(){	
    ...
    function rwd(){
        ...
        if (wid<600){
            $('#m_gnb').off('mouseenter');
            $('#m_gnb').off('mouseleave');
        }
    ...
    $('.btnCall').on('click',function(e){
        ...
        if(!isOn){
            if(isAnimated){
                isAnimated = false;	  
                $('#m_gnb').stop().slideDown(500, function(){
                    isAnimated = true;
                });
                $('.btnCall').addClass('on');
            }    
            ...
            if(isAnimated){
                isAnimated = false;	  
                $('#m_gnb').stop().slideUp(500, function(){
                    isAnimated = true;
		...
		// if(isAnimated){
		// 	isAnimated = false;	  
		// 	$('#m_gnb').stop().slideDown(500, function() {isAnimated = true;} );
		// 	$('.btnCall').addClass('on');
		// }
	});
    ...
    $(window).on('scroll',function(){
        ...
        for(var i =0; i<posArr.length; i++){
            if(scroll>posArr[2] && scroll<posArr[posArr.length-1]){
                $('#header').css('background','rgba(255,255,255,1)');
                $('#header').css('border-bottom','1px solid rgba(0,0,0,0.1)');
                
            }
            if(scroll>posArr[0] && scroll<posArr[2]){
                $('#header').css('background','rgba(0,0,0,1)');
